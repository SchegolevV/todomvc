...
export default class App extends Component {
  ...
  changeTaskStatus = (id) => {
    this.setState(({ taskData }) => {
      ...
      oldItem.status === 'active' ? (newStatus = 'completed') : (newStatus = 'active')

      const newData = taskData.map((task) => (task === oldItem ? { ...task, status: newStatus } : task))
      return {
        taskData: newData,
      }
    })
    ...
    return (
      <section className="todoapp">
        ...
        <section className="main">
          ...
          />
          <Footer count={activeCount} onClearCompleted={this.clearCompleted} onFilterTask={this.filterTask} />
        </section>
      </section>
    )
  }
